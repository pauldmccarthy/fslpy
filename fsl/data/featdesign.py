...
class VoxelwiseEVMixin(object):
    ...
    Some FSL tools (e.g. :fsldocs:`PNM <task_fmri/pnm.html>`) generate
    *compressed* voxelwise EVs, where there is only one voxel per slice. For
    example, if the input data has shape ``(x, y, z, t)``, and slices are
    acquired along the Z plane, voxelwise EV files generated by PNM will have
    shape ``(1, 1, z, t)``.
